...
function CreateLessonForm() {
  ...
  let editorDataFake =
    "<h2>How To Play Piano</h2><h3>Step 1</h3><p>Get a Piano.</p><h3>Step 2</h3><p>Hit the keys.</p";
  const handleSave = (evt) => {
    console.log("handleSave");
    // Send API post
    function handleSubmit(e) {
      e.preventDefault();
      fetch(process.env.FRONTEND_URL + "/api/replies", {
        method: "POST",
    ...
    if (!editorInitialized.current) {
      ...
      function addEditorToDOM() {
        ClassicEditor.create(document.querySelector("#ckeditorroot"), {
          initialData: editorDataFake
            ? editorDataFake
            : "<p>Create your text lesson here.</p>",
        })
          .then((editor) => {
            if (editor) {
              return;
